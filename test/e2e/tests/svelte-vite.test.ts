...
import { startSvelteVite, stopFramework, headingFileContains, type FrameworkServer } from "../helpers/framework.js";
import { openFrontmanUI, sendPrompt } from "../helpers/frontman-ui.js";
import { installSvelteVite } from "../helpers/installer.js";

const PORT = 3014;

describe("Svelte + Vite E2E", () => {
  let browser: Browser;
  ...
  beforeAll(async () => {
    ...
    server = await startSvelteVite(PORT);
  });
  ...
  it("should make a text change via AI prompt", async () => {
    ...
    await sendPrompt(page, 'Change the h1 heading text in src/App.svelte to say "Hello Frontman"');
